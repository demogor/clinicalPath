...
object DIC_DIC_MXFL_2 {
  def main(args: Array[String]): Unit = {
    ...
    spark.sql(
      """
        |drop table if exists clinical_path.DIC_DIC_MXFL_2
      """.stripMargin)
    spark.sql(
      """
        |create table clinical_path.DIC_DIC_MXFL_2 as
        |--药品
        ...
        |  from clinical_path.DIC_DIC_ZLXM t1
        | inner join
        | (select substr(trim(xmbm),1,9) as xmbm,max(xmmc) as xmmc from clinical_path.SUM_TB_DIC_JBYLFW group by substr(trim(xmbm),1,9)) t2
