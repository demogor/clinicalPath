...
object FACT_TEST_LDAJS {
  def main(args: Array[String]): Unit = {
    ...
    val fact_test_lda_df0 = spark.sql("select lsh,djtsy,sjmlmc from clinical_path.FACT_TEST_LDAJS_DATA")
    val separator = "`"
    ...
    /** ****************************************************/
    //得到词与主题的频率关系
    val topics = ldamodel.describeTopics(100)
    ...
    val dic_dic_mxfl_df = spark.sql(
      """
        |SELECT distinct sjmlmc,sdmlbz FROM clinical_path.dic_dic_mxfl_3
      """.stripMargin)
    val fact_test_lda_topic = topics2.selectExpr("topics+1 as topic",
      "key as key",
      "round(value,4) as termWeights").join(vocabulary, $"index" === $"key", "inner").join(dic_dic_mxfl_df, $"xmmc" === $"sjmlmc", "inner").selectExpr(
      "topic", "xmmc", "sdmlbz", "termWeights",
      "row_number() over(partition by topic,sdmlbz order by termweights desc) as rank")
    ...
    val fact_test_lda_ladmodel = ladmodel1.toDF("lsh","topic","topicDistribution1", "topicDistribution2", "topicDistribution3",
      ...
      $"topicDistribution4"/*, $"topicDistribution5", $"topicDistribution6", $"topicDistribution7"*/)
    //保存数据到hive中
    fact_test_lda_topic.write.mode("Overwrite").saveAsTable("clinical_path.fact_test_ldajs_topic")
    fact_test_lda_ladmodel.write.mode("Overwrite").saveAsTable("clinical_path.fact_test_ldajs_ladmodel")
  }
}
