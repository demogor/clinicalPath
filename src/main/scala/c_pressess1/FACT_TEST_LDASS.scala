...
object FACT_TEST_LDASS {
  def main(args: Array[String]): Unit = {
    ...
    val fact_test_lda_df0 = spark.sql("select lsh,djtsy,sjmlmc from clinical_path.FACT_TEST_LDASS_DATA")
    val separator = "`"
    ...
    val dic_dic_mxfl_df = spark.sql(
      """
        |SELECT distinct sjmlmc,sdmlbz FROM clinical_path.dic_dic_mxfl_2
      """.stripMargin)
    ...
    val fact_test_lda_ladmodel = ladmodel1.toDF("lsh","topic","topicDistribution1", "topicDistribution2", "topicDistribution3",
      ...
      $"topicDistribution4"/*, $"topicDistribution5", $"topicDistribution6", $"topicDistribution7"*/)
    //保存数据到hive中
    fact_test_lda_topic.write.mode("Overwrite").saveAsTable("clinical_path.fact_test_ldass_topic")
    fact_test_lda_ladmodel.write.mode("Overwrite").saveAsTable("clinical_path.fact_test_lda_ladmodelfact_test_lda_ladmodel")
  }
}
